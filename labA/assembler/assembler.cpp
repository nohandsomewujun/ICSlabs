...
int RecognizeNumberValue(std::string s) {
    ...
    else if(s[0] == 'X')
    {
        ...
        for(int i = 0;i < s.length();i++)
        {
            if(s[i] >= '0' && s[i] <= '9')
            {
                sum += pow(16,s.length()-1-i)*(s[i]-'0');
            }else
            {
                sum += pow(16,s.length()-1-i)*(s[i]-'A'+10);
            }
        }
        return sum;
    }
...
std::string assembler::TranslateOprand(int current_address, std::string str, int opcode_length) {
    ...
    if (!(item.getType() == vAddress && item.getVal() == -1)) {
        ...
        auto bin2;
        auto address2;
        laaddress = item.getVal();
        address2 = laaddress - current_address - 1;
        bin2 = NumberToAssemble(address2);
        bin2 = bin2.subster(16-opcode_length,opcode_length);
        return bin2;
    }
    if (str[0] == 'R') {
        // str is a register
        ...
        std::string::iterator it;
        it = bin.begin();
        for(int i = 0;i < 16 - opcode_length;i++)
        {
            bin.erase(it);
        }
        return bin;
    ...
    for (int line_index = 0; line_index < file_content.size(); ++line_index) {
        ...
        if (line[0] == '.') {
            ...
            if (pseudo_command == ".ORIG") {
                ...
                file_address[line_index] = line_address;
                std::string fillwhat;
                line_stringstream >> fillwhat;
                line_address++;
            } else if (pseudo_command == ".BLKW") {
                // TO BE DONE
                file_address[line_index] = line_address;
                std::string blockstring;
                line_stringstream >> blockstring;
                auto blocksize = RecognizeNumberValue(blockstring);
                ...
                line_address = line_address + blocksize;
            } else {

...
inline std::string &LeftTrim(std::string &s, const char *t = " \t\n\r\f\v") {
    // TO BE DONE
    //我们接下来要做的是在中字符串中去掉t[0]t[1]t[2]t[3]t[4]t[5]t[6]
    //从左向右修剪
    std::string::iterator it;
    int cnt = 0;
    int len = s.length();
    for(it = s.begin();cnt < len;it++,cnt++)
    {
        for(int j = 0;j < 7;j++)
        {
            if(*it == t[j])
...
inline std::string &RightTrim(std::string &s, const char *t = " \t\n\r\f\v") {
    ...
    return s;
    //存在bug，从左向右可以trim，但从右向左无法tirm
    //上面两个函数存在bug，未弄清函数究竟要修建到什么程度
}
